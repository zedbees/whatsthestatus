...
export function KanbanColumn({ title, status, tasks, onAdd, onDelete, onEdit, onMove, onToggleTimer }: KanbanColumnProps) {
  ...
  return (
    <div className={`flex flex-col h-full bg-column border border-border rounded-2xl shadow-sm min-w-[280px] max-w-xs px-6 ${isActiveColumn ? 'ring-2 ring-primary/20 border-primary/30' : ''}`}>
      ...
      <div className="flex-1 overflow-y-auto min-h-0 max-h-full flex flex-col gap-4 pb-6">
        {tasks.length === 0 ? (
